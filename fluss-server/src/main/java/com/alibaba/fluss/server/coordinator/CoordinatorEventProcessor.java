...
public class CoordinatorEventProcessor implements EventProcessor {
    ...
    public void startup() {
        ...
        // 我们需要在协调器上下文初始化之后、tableManager 中的状态机启动之前发送 UpdateMetadataRequest。
        // 这是因为Tablet 服务器需要从UpdateMetadataRequest 接收活动Tablet 服务器列表，
        // 然后才能处理由replicaStateMachine.startup() 和partitionStateMachine.startup() 生成的LeaderRequest。
        LOG.info("Sending update metadata request.");
        // 更新协调器服务器和所有远程tablet服务器的元数据缓存。
            ...
            return zooKeeperClient
                    ...
                    .map(
                            coordinatorAddress ->
                                    ...
                                    // 我们将 id 设置为 -1，因为协调器服务器存储在 zk 中的 id 现在是一个 uuid
                                    new ServerNode(
                                            -1,
